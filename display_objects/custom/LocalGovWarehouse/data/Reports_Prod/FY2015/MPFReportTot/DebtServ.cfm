...
<span class="pull-right"> 
	<a href="/financial-data/local-government-division/local-government-data/pdfreport/?FileName=/data/reports/FY2015/MPFReportTot/DebtServ.cfm&Code=#url.Code#&CFY=#url.CFY#&PrintIt=Yes&AFRDesiredData=#url.AFRDesiredData# Summary&Format=Multi-Purpose Form&Page=F8" class="btn btn-primary nomargin">
...
<div class="text-center marginb15"><b>Debt Limitations and Future Debt</b></div>
    ...
    <tr>
    <td colspan="3"><input type="checkbox" name="NoLegalDebtLimit"
                           <cfif trim(GetAFRRptPg1.NoDebtLimitReason) is not "">checked</cfif>
                           value="Y" disabled>&nbsp;I certify
                               that <cfoutput>#GetAFRRptPg1.UnitName# #GetAFRRptPg1.Description#</cfoutput>
        does not have Legal Debt Limitation</td></tr>
    <tr>
        <td>&nbsp;</td>
    <td><input type="radio" name="LimitBased" value="Statute" disabled
               <cfif trim(GetAFRRptPg1.NoDebtLimitReason) is "Statute">checked</cfif>>&nbsp;Based on Statute</td>
<td>Explain: <cfif trim(GetAFRRptPg1.NoDebtLimitReason) is "Statute">
    <cfoutput>#GetAFRRptPg1.DebtLimitReason#</cfoutput></cfif></td></tr>
<tr>
    <td>&nbsp;</td>
<td><input type="radio" name="LimitBased" value="Other"
           <cfif trim(GetAFRRptPg1.NoDebtLimitReason) is "Other">checked</cfif>
           disabled>&nbsp;Based on Other Reasons</td>
           <td>Explain: <cfif trim(GetAFRRptPg1.NoDebtLimitReason) is "Other">
    <cfoutput>#GetAFRRptPg1.DebtLimitReason#</cfoutput></cfif></td></tr>
</table>
