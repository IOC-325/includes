...
<span class="pull-right"><cfoutput><cfif url.PrintIt is "no"><a href="/financial-data/local-government-division/local-government-data/pdfreport/?FileName=/data/reports/FY2013/RegOnlyReportTot/RegOnlyPg2c.cfm&Code=#url.Code#&CFY=#url.CFY#&PrintIt=Yes&AFRDesiredData=#url.AFRDesiredData# Summary&Format=Registration Form&Page=2" class="btn btn-primary btn-lg"><span class="glyphicon glyphicon-print"></span>
...
<tr><td width="30%"><font size="-1"><cfif url.PrintIt is "no"><a href="/financial-data/local-government-division/local-government-data/processsearchresults/?DisplayMode=GetAFR&AFRDesiredData=#url.AFRDesiredData#&Code=#url.Code#&CFY=#url.CFY#&Menu=No" target="_New"></cfif>#GetAFRRptPg1.UnitName# #GetAFRRptPg1.Description#<cfif url.PrintIt is "no"></a></cfif></font></td><td width="15%" align="right"><font size="-1">#NumberFormat(GetAFRRptPg1.Appropriations,"$999,999,999,999,999,999")#</font></td><td width="15%"><font size="-1">#DateFormat(GetAuditInfo.FYEnd,"MM")#/#DateFormat(GetAuditInfo.FYEnd,"DD")#</font></td></tr>
</cfoutput>
<cfoutput query="GetComponentUnitInfo">
<cfset SubApprop = #iif(SubApprop is "",0,SubApprop)# + #iif(Amount is "",0,Amount)#>
<tr><td width="30%"><font size="-1"><cfif url.PrintIt is "no"><a href="/financial-data/local-government-division/local-government-data/processsearchresults/?DisplayMode=GetAFR&AFRDesiredData=#url.AFRDesiredData#&Code=#url.Code#&CFY=#url.CFY#&Menu=No" target="_New"></cfif>#ComponentUnit#<cfif url.PrintIt is "no"></a></cfif></font></td><td width="15%" align="right"><font size="-1">#NumberFormat(Amount,"$999,999,999,999,999")#</font></td><td width="15%"><font size="-1">#DateFormat(FYEnd,"MM")#/#DateFormat(FYEnd,"DD")#</font></td></tr>
</cfoutput>
<cfoutput>
