...
<!--- If the search is NOT an All Unit or AFR Search, then use the form that automatically drops down the list upon cursor entry --->   
    ...
    * html .ui-autocomplete {
    ...
    $(function() {
        var defaultCFCMethod = "<cfoutput>#variables.CFCMethod#</cfoutput>";
        var cfcMethod = ($("#unitType").val() !== "") ? $("#unitType").val() : defaultCFCMethod;

        $("#unitType").on("change", function(){
            var unitType = $(this).val();
            cfcMethod = (unitType) ? unitType : defaultCFCMethod;
            var newSource = "/Queries/LocalGoveWarehouseSearchService.cfc?method=" + cfcMethod + "&returnformat=json";

            $("#category").val("");
            $("#category").autocomplete('option', 'source', newSource);
        ...
        $("#category").autocomplete({
            ...
            source: "/Queries/LocalGoveWarehouseSearchService.cfc?method=" + cfcMethod + "&returnformat=json",
            </cfoutput>
    ...
    <!--- <h3>Currently Searching <span class="SelectedFilter">All Governments</span></h3>
    <ul class="filterNav text-center">
        <li>Filter by: </li>
        <li><a href="#" class="disabled">All Governments</a></li>
        <li><a href="#">Counties</a></li>
        <li><a href="#">Municipalities</a></li>
        <li><a href="#">Townships</a></li>
        <li><a href="#">Special Purpose Districts</a></li>
    </ul> --->
    ...
    <cfif cgi.SCRIPT_NAME eq "/FilteredSearching.cfm">
        ...
        <div class="row margintop">
            <div class="col-xs-12 col-sm-4 label-div">
                <div class="btn-label-grey btn-label label-gray">Unit Type</div>
            </div>
            <div class="col-xs-12 col-sm-6 default-select">
                <select id="unitType" class="select2">
                    <option value="">All Units</option>
                    <option value="getCounties">Counties</option>
                    <option value="getMunicipalities">Municipalities</option>
                    <option value="getTownships">Townships</option>
                    <option value="getSpecialTaxingDistricts">Special Purpose Districts</option>
                </select>
            </div>
        </div>
        <div class="row margintop">
