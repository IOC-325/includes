<!--- 
    ...
    Date:      01/14/20
    Author:    Zac Ely
    Changes:   Added part numbers to the link if the instrument's TIF file has multiple parts
    CodeReviewed and Approved by ASP 2020-01-15
---><!--- 
    SR:        19198
    Date:      01/22/20
    Author:    Zac Ely
    ...
    <cfif CheckRecordLoc.recordcount>
        ...
        <cfloop query="CheckRecordLoc"> 
			...
			<cfif trim(CheckRecordLoc.Approved) is not "">
                <a target="_blank" href="#application.TIFpdfLoc#FY#x#/#left(url.Code,3)##mid(url.Code,5,3)##right(url.Code,2)#/<cfif trim(folderpath) is not "">#folderpath#/</cfif>#FileName#.pdf">
            </cfif>
            ...
            <!--- SR 19197: If previous or next instrument is the same as the current instrument then add part number to link label --->
            <cfif (CheckRecordLoc.currentRow gt 1 and CheckRecordLoc.Instrument eq CheckRecordLoc.Instrument[currentRow-1])
                OR (CheckRecordLoc.currentRow+1 lte CheckRecordLoc.recordCount and CheckRecordLoc.Instrument eq CheckRecordLoc.Instrument[currentRow+1])>
                <cfset fileSplit = listToArray(fileName, "_")>
                <cfset filePart = "Part " & fileSplit[arrayLen(fileSplit)]>
            <cfelse>
                <cfset filePart = "">
            </cfif>

            #EncodeForHTML(TmpDesc & " " & filePart)#
