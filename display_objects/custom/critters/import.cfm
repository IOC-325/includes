<cftry>
  ...
  <cfset cData = csvToArray( 'E:\WEB\Main\comptroller\includes\display_objects\custom\critters\shelters.csv' ) />

  <cfset arrIndex = 0 />
  <cfloop array="#cData#" index="i">
    <cfset arrIndex++ />
    ...
    <cfset sZip = i[4] />
    <cfset sPhone = i[5] />
    <cfset sSecondaryPhone = i[6] />
    <cfset sWebsite = i[7] />
    <cfset sPetFinder = i[8] />
    <cfset sLatitude = i[9] />
    <cfset sLongitude = i[10] />

    <cfif len( trim(sName) )>
      <cfset node = $.getBean('content').loadBy( title = sName )>

      <cfif node.getIsNew() eq 1 and arrIndex neq 1>
        <cfset node.setType('Page')>
        <cfset node.setSubType('Animal Shelter')>
        <cfset node.setParentId('510378F9-5056-93B5-5E6E323BD6A6ECB0')>
        <cfset node.setTitle( sName )>
        <cfset node.setStreetAddress( sAddress )>
        <cfset node.setCity( sCity )>
        <cfset node.setState( 'IL' )>
        <cfset node.setZipCode( sZip )>
        <cfset node.setPhone( sPhone )>
        <cfset node.setSecondaryPhone( sSecondaryPhone )>
        <cfset node.setWebsite( sWebsite )>
        <cfset node.setPetFinder( sPetFinder )>
        <cfset node.setLatitude( sLatitude )>
        <cfset node.setLongitude( sLongitude )>
        <cfset node.setApproved(1) />
        <cfset node.setActive(1) />
        <cfset node.setDisplay(1) />
        <cfset node.save() />
      </cfif>
    </cfif>
