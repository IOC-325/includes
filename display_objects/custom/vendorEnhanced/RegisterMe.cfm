...
<!--- Verifies that the user is signing up for a FEIN which belongs to only a user or Deduction --->
        ...
        <cfif (CheckValidVendor.recordcount gt 1) and form.Number_Code is "">
            ...
            <!---<form role="form" class="form-horizontal" name="VendorRemit"
                  action="https://illinoiscomptroller.gov/vendors/enhanced-vendor-remittance/enhanced-vendor-registration/" method="post">--->
            
            <form role="form" class="form-horizontal" name="VendorRemit" action="https://2018-uat.illinoiscomptroller.gov/vendors/enhanced-vendor-remittance/enhanced-vendor-registration/" method="post">
...
--->
                ...
                <input type="hidden" name="NotificationType" value="#TmpNotificationType#">
...
<!--- Verifies that if the user has signed up for text notifications that he/she has given us their cell phone provider --->
                            <cfelseif trim(form.PaymentNotification) is "Y" and form.NotificationType is "Text" and (form.CellProvider is "None")>
                            <div class="alert alert-danger col-sm-12"><span class="glyphicon glyphicon-remove"
                                                                            aria-hidden="true"></span>
                            <b>We can not register #trim(form.FirstName)# #trim(form.LastName)#
                                as an authorized user of the Secure Enhanced Vendor Remittance application since you
...
<!--- Verifies the user has entered a correct Bank Account Number and Routing Number for the Vendor we have on file --->
                            ...
                            <cfStoredProc datasource="#application.VendorPayments#" Procedure="Add_RejRegistration">
                                ...
                                <cfprocparam type="In" cfsqltype="cf_sql_varChar" dbvarname="@RejReason" value="#CheckValidVendorRetVal#">
                                <cfprocparam type="Out" cfsqltype="cf_sql_integer" dbvarname="@RetVal" variable="CheckNoVendorCoordinatorRetVal">
                                <cfprocparam type="Out" cfsqltype="cf_sql_varchar" dbvarname="@RetMsg" variable="CheckNoVendorCoordinatorRetMsg">
                                <cfprocresult name="CheckNoVendorCoordinator">
                            </cfstoredproc>
                                    ...
                                    <CFMail
                                        ...
                                        <P>This is an automated E-Mail sent to you by the Illinois Office of the
                                            ...
                                            been registered as a user of this site.</b><br><br>Please click on
                                                <a href="https://#application.EVRSite#/vendors/enhanced-vendor-remittance/create-password/?ValidateEmail=Yes&TaxpayerInfoNo=#urlEncodedformat(encrypt(TmpTaxpayerIdNo, application.theKey, application.theAlgor, application.TheEncode))#&Email=#urlEncodedFormat(form.EMail)#&Number_Code=#urlEncodedformat(TmpNumberCode)#">
                                        ...
                                        Address.    https://#application.EVRSite#
...
--->
                    ...
                    <cfelse>
                        <div class="alert alert-danger col-sm-12"><span class="glyphicon glyphicon-remove"
                                                                        aria-hidden="true"></span>
                        <b>We're sorry, we have had more then 3 registration attemptes with critical errors on #dateformat(now(), "m/d/yyyy")# for your vendor, so we can not attempt to register you again until #dateformat(dateadd("d", 1, now()), "m/d/yyyy")#.  If on #dateformat(dateadd("d", 1, now()), "m/d/yyyy")# you continue to have problems trying to register, please E-Mail us at <a href="mailto:#application.VendorRemitEmail#">#application.VendorRemitEmail#</a>.</b>
                    </div>
                        <cfset ErrorNo = "17">
                        <cfStoredProc datasource="#application.VendorPayments#" Procedure="Add_RejRegistration">
                            <cfprocparam type="In" cfsqltype="cf_sql_varchar" dbvarname="@Email" value="#form.Email#">
...
--->
    ...
    <h3 class="margintop30 bold">FINANCIAL INSTITUTION INFORMATION</h3>
