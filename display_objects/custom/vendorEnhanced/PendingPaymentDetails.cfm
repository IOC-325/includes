...
<CFQUERY NAME="WarrLinesSumm" DATASOURCE="DB2PRD">
	SELECT 	wh.TRANS_EXP.Fiscal_Year, wh.TRANS_EXP.ACCEPTANCE_DATE , SUM(wh.TRANS_EXP.EXPENDED_AMT) AS EXPENDED_AMT
	FROM 	wh.TRANS_EXP , wh.AGENCY , wh.Approp
	WHERE 
			wh.TRANS_EXP.AGENCY = wh.AGENCY.AGENCY 
	  AND   wh.TRANS_EXP.FISCAL_YEAR = wh.AGENCY.FISCAL_YEAR
      ...
      AND 	wh.TRANS_EXP.VENDOR_TIN = <cfqueryparam cfsqltype="cf_sql_varchar" value="#VendTin#" />
      AND   wh.TRANS_EXP.Transaction_Number = <cfqueryparam cfsqltype="cf_sql_varchar" value="#Transaction_Number#" />
    GROUP BY wh.TRANS_EXP.Fiscal_Year, wh.TRANS_EXP.ACCEPTANCE_DATE
</CFQUERY>
...
<div class="marginb20"><cfoutput><a href="/vendors/enhanced-vendor-remittance/pending-payments/?Mode=Warrants&VendTINName=#VendTINName#&StartRow=#StartRow#&Max=#URLEncodedFormat(Max)#" id="ReturnBack"><i class="glyphicon glyphicon-share-alt"  ></i> Return Back</a></cfoutput></div>
<div name="FormWrapper" id="FormWrapper" class="table-responsive">
<TABLE class="table table-striped table-bordered vendor-table">
    <thead>
        <tr>
            <th colspan="5">Voucher Number: <cfoutput>#Transaction_Number#</cfoutput></th>
        </tr>
    </thead>
	<CFOUTPUT QUERY="WarrLinesSumm">
	<TR id="lightcolor">
	...
	<TR id="lightcolor">
		...
		<TD ALIGN="RIGHT" id="WarrantTotal4">#DollarFormat(EXPENDED_AMT)#</TD>
		<TD COLSPAN="3">&nbsp;</TD>
	</TR>
