<!--- DATE: 11/15/2019
      MEGAN CUSEY
      SR # 19185 - Added lifetime contract amount field when the contract is a Multi Year Contract.
---->
...
<cfif MutliYear eq "Yes">
    <!--- QUERY LIFETIME CONTRACT AMOUNT --->
    <cfinvoke component="ledger.includes.display_objects.custom.StateContractsSQL.Component.StateContract_DB2"
        method="getLifetimeContractAmount"
        returnvariable="qLifetimeContractAmount" >
        <cfinvokeargument name="contract_number" value="#URL.ID#"/>   
        <cfinvokeargument name="agency_number" value="#URL.ID3#"/>
    </cfinvoke>
</cfif>
            ...
            <td>
            	<table>
                	<tr>
                        ...
                        <td style="padding:0 0 0 20px"><a href="/financial-data/state-expenditures/contracts/payment-details?ID=#URL.ID#&ID2=#URL.ID2#&ID3=#URL.ID3#&ID4=#URL.ID4#&varClassCode=#URL.varClassCode#&VName=#URLEncodedFormat(URL.VName)#">Show Payments</a></td>
    ...
    <td>
        <table>
           ...
           <tr>
            ...
            <cfif MutliYear EQ "Yes">
           <tr>
                <td  align="right">Lifetime Contract Amount:</td>
                <td style="padding:0px 20px 0px 20px" align="right"><B>#qLifetimeContractAmount#</B></td>
            </tr>
            </cfif>
        </table>
    </td>
